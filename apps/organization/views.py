    ...
    def get(self, request):
        ...
        if search_keywords:
            all_orgs = all_orgs.filter(Q(name__icontains=search_keywords) | Q(
                desc__icontains=search_keywords))  # i 不区分大小
...
class OrgHome(View):
    ...
    def get(self, request, org_id):
        ...
        if request.user.is_authenticated and UserFavorite.objects.filter(
                user=request.user, fav_type=2, fav_id=org_id):
            has_favor = True
...
class FavorView(View):
    ...
    def post(self, request):
        ...
        exist_records = UserFavorite.objects.filter(
            user=request.user, fav_id=int(favor_id), fav_type=int(favor_type))
        ...
        if search_keywords:
            all_teachers = all_teachers.filter(
                Q(
                    ...
                    work_company__icontains=search_keywords) | Q(
                    work_position__icontains=search_keywords))  # i 不区分大小
