...
class UserProfile(AbstractUser):
    ...
    phone_num = models.CharField(verbose_name=u"手机",max_length=11, null=True, blank=True)
    # ImageField instances are created in your database as varchar columns
    # with a default max length of 100 characters.
...
class EmailVerifyRecord(models.Model):
    ...
    send_type = models.CharField(verbose_name=u"验证码类型",
        choices=(("register", u"注册"), ("forget", u"找回密码"), ("change", u"更换邮箱")), max_length=20)
    send_time = models.DateTimeField(verbose_name=u"发送时间",default=datetime.now)
    # datetime.now() 编译时生成；不加括号  实例化时生成当前时间
